...
ggsave(plot = source_diet_map, filename = here("figures/source_vs_diet_visualisation_map_ge_allocation.jpg"), device = "jpg", dpi = 600, width = 18, height = 15.5, units = "cm")
```
...
(both_diets_agg_p <- ggplot(data = marine_plant_df |> mutate(diet = case_when(diet == "plant diet" ~ "Plant-\ndominant\nfeed",
  ...
  theme_pubr()+
  geom_boxplot(notch = TRUE, colour = "grey40", width = 0.3, alpha = 0, linewidth=0.5)+
...
both_diets_disagg_p <- 
  ...
  geom_point(position = position_dodge(width = 0.75), size=0.8)+
  geom_errorbar(aes(x = pressure, ymin = mean-sd, ymax = mean+sd), position = position_dodge(width=0.75), width = 0, linewidth = 0.5)+
...
ggsave(filename = here("figures/disaggregation_of_pressures_ge_allocation.jpg"), width = 8.9, height=8, units = "cm", device = "jpg", dpi = 600)
```
