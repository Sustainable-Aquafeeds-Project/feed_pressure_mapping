...
map(md_crop_r_list, \(this_file){
  ...
  saveName <- sprintf(here("data/spatial/%s/int/%s_N_km2_%s.tif"), this_diet, this_ingredient, this_source)
  
  if(!file.exists(saveName)){
    
    message("processing... ", basename(this_file))
    
    this_N_value_kg <- readRDS(here("data/tidy_data/LCA/crop_lca.rds")) |> filter(FAOSTAT_name == this_raw_material &iso3c == this_source & impact_category %in% c("Marine eutrophication", "Eutrophication, marine")) |> pull(value) |> mean()
...
map(md_crop_r_list, \(this_file){
  ...
  saveName <- sprintf(here("data/spatial/%s/int/%s_P_km2_%s.tif"), this_diet, this_ingredient, this_source)
  
  if(!file.exists(saveName)){
    
    message("processing... ", basename(this_file))
    
    this_P_value_kg <- readRDS(here("data/tidy_data/LCA/crop_lca.rds")) |> filter(FAOSTAT_name == this_raw_material &iso3c == this_source & impact_category %in% c("Freshwater eutrophication", "Eutrophication, freshwater")) |> pull(value) |> mean()
