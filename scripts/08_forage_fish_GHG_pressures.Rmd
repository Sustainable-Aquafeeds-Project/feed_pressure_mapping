...
ghg_raw_r_list <- 
  ...
  saveName <- sprintf(here("data/spatial/%s/int/forage_fish_ghg_raw_%s_%s.tif"), this_diet, this_diet, this_source)
...
Now we need to rescale all values - but will wait until we know if we will rescale within feed contexts

```{r}



pressures <- list.files(file.path("/mnt/rdsi/github/feed_pressure_mapping/data/spatial/marine_diet/pressures/"), full.names = TRUE)


plot(rast(pressures[1:3]))
