...
map(md_crop_r_list, \(this_file){
  ...
  if(!file.exists(saveName)){
    ...
    this_h2o_value_kg <- readRDS(here("data/tidy_data/LCA/crop_lca.rds")) |> filter(FAOSTAT_name == this_raw_material &iso3c == this_source & impact %in% c("Water consumption") & allocation == this_allocation_method) |> pull(value) 
...
map(pd_crop_r_list, \(this_file){
  ...
  if(!file.exists(saveName)){
    ...
    this_h2o_value_kg <- readRDS(here("data/tidy_data/LCA/crop_lca.rds")) |> filter(FAOSTAT_name == this_raw_material &iso3c == this_source & impact %in% c("Water consumption", "Water use") & allocation == this_allocation_method
                                                                                    ) |> pull(value) |> mean()
