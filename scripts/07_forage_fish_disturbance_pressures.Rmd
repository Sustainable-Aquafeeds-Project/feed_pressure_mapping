...
rescale_value <- quantile(unlist(forage_effort_km2_r_list), 1)
...
catch_r_list <- list.files(here("data/spatial/fisheries-catch"), full.names = TRUE) |> map(rast)



#######THIS NEEDS FIXING SO THE _ea_npp prefix doesn't keep adding to the file extension

#Convert to equal area, adjust by npp and save
map(catch_r_list, \(this_r){
  
  saveName <- gsub(pattern = ".tif", replacement = "_ea_npp.tif", sources(this_r))
...
#marine-diet
benthic_destruction_md <- list.files(here("data/spatial/marine_diet"), pattern = "benthic", full=TRUE)
biomass_removal_md <- list.files(here("data/spatial/marine_diet"), pattern = "biomass", full=TRUE)
...
combined_raster_list <- 
  ...
  writeRaster(x = this_combined_r, filename = sprintf(here("data/spatial/marine_diet/forage_fish_disturbance_pressure_marine_diet_%s.tif"), this_source))
...
combined_raster_list2 <- 
 ...
 writeRaster(x = this_combined_r, filename = sprintf(here("data/spatial/plant_diet/forage_fish_disturbance_pressure_plant_diet_%s.tif"), this_source))
