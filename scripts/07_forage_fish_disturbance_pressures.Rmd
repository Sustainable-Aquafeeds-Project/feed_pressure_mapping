...
map(.x = prop_raster_files, .f = \(this_file){
  ...
  saveName <- gsub(pattern = "catch", replacement = "effort", tools::file_path_sans_ext(basename(this_file)))
  saveName <- gsub(pattern = "_prop", replacement = "", saveName)
  
  this_source <- substr(saveName, nchar(saveName)-3+1, nchar(saveName) )
  ...
  this_effort_raster <- lapp(this_stack_r, fun = \(x, y){x * y}, filename = sprintf(here("data/spatial/03-fisheries-effort/$.tif"), saveName))
