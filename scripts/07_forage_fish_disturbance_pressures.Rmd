...
map(.x = forage_effort_km2_r_files, .f = \(this_file){
  ...
  if(!file.exists(saveName)){
    ...
    rescale_value <- quantile(c(terra::values(this_rast, na.rm=TRUE)), 0.9999)
...
combined_raster_list <- 
  ...
  writeRaster(x = this_combined_r, filename = sprintf(here("data/spatial/marine_diet/forage_fish_disturbance_pressure_marine_diet_%s.tif"), this_source), overwrite=TRUE)
...
combined_raster_list2 <- 
 ...
 writeRaster(x = this_combined_r, filename = sprintf(here("data/spatial/plant_diet/forage_fish_disturbance_pressure_plant_diet_%s.tif"), this_source), overwrite=TRUE)
