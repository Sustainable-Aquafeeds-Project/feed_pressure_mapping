---
title: "18 CPI permutation (energetic allocation)"
...
#source files - not sure why this isn't working
...
(both_diets_agg_p <- ggplot(data = marine_plant_df |> mutate(diet = case_when(diet == "plant diet" ~ "Plant-\ndominant\nfeed",
  ...
  geom_point(data = both_diet_means, aes(y = diet, x=mean_CPI), colour = "red", size = 1.8, alpha = 0.7, shape = 0)+
...
test_plot <- ggplot(data= summarised_pressure_disagg_df, aes(x = pressure, y = mean, fill = diet))+
  ...
  coord_polar(clip="off") +theme_bw()


y_breaks <- ggplot2::ggplot_build(test_plot)$layout$panel_params[[1]]$r.major

#USE THIS DATA FRAME IF YOU WANT PROPORTIONAL COMPARISONS
...
#USE THIS DATA FRAME IF YOU WANT ABSOLUTE COMPARISONS
...
both_diets_disagg_p <- 
  ...
  labs(y = "Absolute CPI contribution")+
  guides(fill = guide_legend(override.aes = list(shape = NA)))+
  annotate("text", x = 4.5, y = y_breaks, label = as.character(y_breaks), size=2, colour="grey10")
