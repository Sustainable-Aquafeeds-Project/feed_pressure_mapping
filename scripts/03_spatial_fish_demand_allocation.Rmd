...
```{r}
forage_catch <- readRDS(here("data/raw_data/fisheries/spatial_forage_catch_2017.rds")) |> filter(ReportedIND >0)

(forage_catch_summary <- forage_catch |> 
  group_by(TaxonName, CommonName) |> 
  summarise(total_catch = sum(ReportedIND, na.rm=TRUE)) |> 
  arrange(-total_catch) |> 
...
forage_catch |> 
  ...
  mutate(mass_value = case_when(#when the na group is ones we know from trimmings - assign speciically from Cashion 2016 data
                                      ...
                                      is.na(mass_value) & ingredient == "Fishmeal" ~ globalav_embodiedfish_fm_mass,
                                      is.na(mass_value) & ingredient == "Fish oil" ~ globalav_embodiedfish_fo_mass,
                                      TRUE ~ mass_value),
         ...
         ge_value = case_when( is.na(ge_value) & TaxonName == "Sardina pilchardus" &ingredient == "Fishmeal" ~ 1.66,
                                      ...
                                      is.na(ge_value) & ingredient == "Fishmeal" ~ globalav_embodiedfish_fm_ge,
                                      is.na(ge_value) & ingredient == "Fish oil" ~ globalav_embodiedfish_fo_ge,
                                      TRUE ~ ge_value)) |> 
...
#Find out how much fishmeal and oil is available globally from trimmings and forage
