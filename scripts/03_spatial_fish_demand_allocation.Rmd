...
(foragefish_cfs <- read_csv(here("data/raw_data/allocation/embodied_fish_allocation_tidy.csv")) |> 
  select(common_name, sci_name, ingredient, ge_value, mass_value) |> 
...
forage_catch |> 
  ...
  left_join(foragefish_cfs, by = c("ingredient" , "TaxonName" = "sci_name", "CommonName" = "common_name")) |> 
  #fill in NA values through species group values for mass and energetic allocation
  mutate(mass_value = case_when(#when the na group is ones we know from trimmings - assign specifically from Cashion 2016 data
  ...
  map_df(.x =  c("marine_diet", "plant_diet"), .f = \(this_diet){
    ...
    this_df <- forage_fmfo_yield |> 
      mutate(diet = this_diet,
             ...
             fm_embodied_fish_mass = distributed_fm_demand_mass * min_fm_forage_prop * mass_value_fishmeal ,
             fo_embodied_fish_mass = distributed_fo_demand_mass * min_fo_forage_prop * mass_value_fish_oil,
             fm_embodied_fish_ge = distributed_fm_demand_ge * min_fm_forage_prop * ge_value_fishmeal,
             fo_embodied_fish_ge = distributed_fo_demand_ge * min_fo_forage_prop * ge_value_fish_oil,
...
trim_supply_demand_by_diet <- 
  map_df(.x =  c("marine_diet", "plant_diet"), .f = \(this_diet){
    ...
    this_trimmings_df <- trim_fmfo_yield 
    
    these_weight_sums <- 
      map_df(this_demand_df |> 
      group_by(fao_area_code) |> 
      group_split(), \(this_df){
  ...
  unnest(cols = c(data, prop_fm_mass, prop_fo_mass, prop_fm_ge, prop_fo_ge)) |> 
      mutate(
        #distribute fishmeal and oil demand according to its distribution and the weight of that group in trimmings
             distributed_fm_demand_mass =  weighted_fm_needed * prop_fm_mass,
             distributed_fo_demand_mass = weighted_fo_needed * prop_fo_mass,
             distributed_fm_demand_ge = weighted_fm_needed * prop_fm_ge,
             distributed_fo_demand_ge = weighted_fo_needed * prop_fo_ge,
            
             #calculate embodied demand
             #calculated the embodied fish from fm and fo
             fm_embodied_fish_mass = distributed_fm_demand_mass * max_fm_trim_prop * mass_byproduct_cf_fishmeal,
             fo_embodied_fish_mass = distributed_fo_demand_mass * max_fo_trim_prop * mass_byproduct_cf_fish_oil,
             fm_embodied_fish_ge = distributed_fm_demand_ge * max_fm_trim_prop * ge_byproduct_cf_fishmeal,
             fo_embodied_fish_ge = distributed_fo_demand_ge * max_fo_trim_prop * ge_byproduct_cf_fish_oil,
             #calculate the total fmfo demand
             total_fmfo_demand_mass = distributed_fm_demand_mass + distributed_fo_demand_mass,
             total_fmfo_demand_ge = distributed_fm_demand_ge + distributed_fo_demand_ge,
             #calculate the total embodied fish
...
data.table::fread(here("data/large_data/embodied_foragefish_per_cell.csv")) |> pull(fao_area_code) |> unique()
data.table::fread(here("data/large_data/embodied_fishfromtrimmings_per_cell.csv")) |> pull(fao_area_code) |> unique()
