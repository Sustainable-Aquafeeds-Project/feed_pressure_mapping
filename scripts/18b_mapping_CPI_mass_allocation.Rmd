...
countries <-    ne_countries(scale = "medium", returnclass = "sf") |> st_transform(crs = gp_crs)
bbox <- ne_download(scale = 50, type = "wgs84_bounding_box", category = "physical", returnclass = "sf") |> st_transform(crs = gp_crs)
coastline <- ne_coastline(scale = 50, returnclass = "sf") |> st_transform(crs = gp_crs)
select <- dplyr::select
values <- terra::values
```
...
marine_source_ingredient <- 
  marine_diets |> 
  select(ingredient) |> 
  mutate(source_1 = c(1, 1, 3, 4, 2, 5, 1),
         source_2 = c(2, 3, 1, 2, 4, 2, 3),
         source_3 = c(3, 5, 5, 1, 3, 2, 4))|> 
...
plant_source_ingredient <- 
  plant_diets |> 
  select(ingredient) |> 
 mutate(source_1 = c(1, 1, 2, 4, 4, 3, 3, 1, 3, 4, 2, 2, 1),
         source_2 = c(2, 2, 3, 3, 1, 2, 3, 3, 5, 1, 3, 5, 2),
         source_3 = c(3, 3, 1, 5, 5, 1, 4, 2, 1, 2, 1, 3, 5)) |> 
   mutate(row = row_number()) |> 
  group_split(row) 
# 
...
(marine_sourcing_p1 <- 
  ...
  scale_fill_gradientn(
    ...
    annotate("text", x = -18000000, y = sum(c(-9008064, 9009955))|> mean(), label = "Raw material origin\ncombination 1", angle = 90)+
...
# ggsave(plot = source_diet_map, filename = here("figures/source_vs_diet_visualisation_map_mass_allocation.jpg"), device = "jpg", dpi = 600, width = 18, height = 19, units = "cm")
```
...
```{r}
  ...
  ggplot()+
  geom_sf(data = bbox |> st_transform(crs = moll_crs), colour="grey75", fill= "aliceblue", alpha=0.6, size = 0.3)+
  geom_sf(data = countries |> st_transform(crs = moll_crs), fill="grey90", colour= "white", linewidth=0.15)+
  geom_tile(data = marine_diet3_df, aes(x = x, y=y, fill=sum))+
  geom_sf(data = coastline |> st_transform(crs = moll_crs), colour="grey45", linewidth= 0.15)+
...
Make map into robinson projection for website
```{r}

moll_rast <- rast(crs = moll_crs)
values(moll_rast) <- 1:ncell(moll_rast)


marine_rast <- rast(marine_diet3_df, type="xyz", crs = gp_crs)

project(marine_rast, moll_rast)
