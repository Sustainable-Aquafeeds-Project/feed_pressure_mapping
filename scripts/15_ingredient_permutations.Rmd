...
md_df_list <- vector(mode="list", length = 2500)
...
for(i in 1:2500){
  ...
  this_diet_df <- 
    ...
    if(this_ingredient != "fishmeal"  & this_ingredient != "fish oil"){
    ...
    source_select <- sample(x=1:3, replace = FALSE, size=1) 
...
Given the number of unique combinations varies from marine to plant diet - here I take the first 2000 diets generated from each and save them for future use.

```{r}

source_combinations_md <- source_combinations_md |> select(1:2001)

source_combinations_pd <-  source_combinations_pd |> select(1:2001)
