...
(plausible_crop_sourcing_list <- crop_demand_production_compare |> select(ingredients, diet, FAOSTAT_name, source_iso3c, total_crop_demand, national_production, demand_production_ratio, check) |>  filter(check == FALSE & ingredients!= "guar meal") |> group_split(ingredients))
...
#Yes - guar has only 1 which we knew and peas is the next lowest with 3
...
crop_sourcing_countries <- 
  ...
  summarise(min = min(cum_value),
            ...
            max = max(cum_value)) |> 
  add_row(raw_material = "Guar seed", min = guar_seed_cum_value, mid = guar_seed_cum_value, max = guar_seed_cum_value) |> 
  pivot_longer(names_to = "value_range", values_to = "cum_value", -raw_material) |> 
  left_join(plausible_crops_w_impact |> 
  group_by(raw_material, iso3c) |> 
  summarise(cum_value = sum(value)), by = c("raw_material", "cum_value"))
