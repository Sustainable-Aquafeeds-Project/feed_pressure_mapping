...
#select depending if using rdsi or one drive
#rdsi_dir <- rdsi_dir
...
each_file <- list.files(here("data/raw_data/LCA"), pattern = "allocation", full.names = TRUE)[[1]]
...
lca_all_crops <- 
  ...
  map_df(\(each_file){
    ...
    this_allocation_method <- case_when(grepl("ge_allocation", each_file) ~ "Energetic allocation",
...
The NPP data was taken from the Cumulative Human Impacts Project at
NCEAS. Downloaded from the aurora server.

The raster has been gapfilled to make all the NA values mean
productivity values
...
npp_2019_monthly <- list.files(file.path(rdsi_dir, "raw_data/vgpm-npp/vgpm.m.2019"), full.names = TRUE) |> rast()

NAflag(npp_2019_monthly) <- -9999 #flag the NAs
values(npp_2019_monthly) <- values(npp_2019_monthly)*0.365242199 #conversion from mg C m-2 day-1 to tonnes C km-2 yr-1
ext(npp_2019_monthly) <- ext(rast()) #set global wgs84 extent


npp_2019_monthly_gf <- focal(npp_2019_monthly_0.5, w=9, fun = mean, na.policy = "only", na.rm=TRUE)

npp_2019_monthly_0.5 <- npp_2019_monthly_gf |> resample(rast(res=0.5)) #convert to half degree

#mean npp with gapfilling
app(npp_2019_monthly_0.5, fun = mean, na.rm =TRUE) |> focal(w=3, fun = mean, na.policy = "only", na.rm=TRUE, filename = here("data/spatial/00-net-primary-productivity/annual_mean_npp_2019_gf_wgs.tif"), overwrite=TRUE)

#median npp with gapfilling
app(npp_2019_monthly_0.5, fun = median, na.rm =TRUE) |> focal(w=3, fun = mean, na.policy = "only", na.rm=TRUE, filename = here("data/spatial/00-net-primary-productivity/annual_median_npp_2019_gf_wgs.tif"), overwrite=TRUE)

# max npp with gapfilling
app(npp_2019_monthly_0.5, fun = max, na.rm =TRUE) |> focal(w=3, fun = mean, na.policy = "only", na.rm=TRUE, filename = here("data/spatial/00-net-primary-productivity/annual_max_npp_2019_gf_wgs.tif"), overwrite=TRUE)

#min npp with gapfilling
app(npp_2019_monthly_0.5, fun = min, na.rm =TRUE) |> focal(w=3, fun = mean, na.policy = "only", na.rm=TRUE, filename = here("data/spatial/00-net-primary-productivity/annual_min_npp_2019_gf_wgs.tif"), overwrite=TRUE)
