...
Create all shapefiles for each taxon
```{r}

taxon_list <- list.files(file.path(iucn_dir, "spp_shapefiles"))[-grep(pattern = "hybas_table", list.files(file.path(iucn_dir, "spp_shapefiles")))] #excluding birds of the world and hybas tables
...
#RUN 1
map(.x = taxon_list[1:3], .f = \(this_taxon){
...
#RUN 2
map(.x = taxon_list[4:6], .f = \(this_taxon){
