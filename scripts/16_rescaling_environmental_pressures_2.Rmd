...
md_reprojected_rasters <- list.files(path = here("data/spatial/marine_diet/int"), pattern = "disturbance_moll", full.names =TRUE)
...
maximum_disturbance_scores_md <-  
  ...
  these_ingredient_rasters <- md_reprojected_rasters[grepl(this_ingredient, md_reprojected_rasters) & grepl(this_allocation_method, md_reprojected_rasters)]
  
  max_disturbance <- these_ingredient_rasters |> map(rast) |> map(no_na_values) |> map(sum) |> unlist() |> max()
...
#PLANT DIETS 
pd_reprojected_rasters <- list.files(path = here("data/spatial/plant_diet/int"), pattern = "disturbance_moll", full=TRUE)
...
maximum_disturbance_scores_pd <-  
  ...
  these_ingredient_rasters <- pd_reprojected_rasters[grepl(this_ingredient, pd_reprojected_rasters) & grepl(this_allocation_method, pd_reprojected_rasters)]
