...
marine_plant_pressure_disagg_df <- 
  ...
  mutate(pressure = factor(pressure, levels = c("")))
...
ggplot(data = marine_plant_pressure_disagg_df |> 
  ...
  aes(x = sum, y = reorder(pressure, sum), fill = diet)+
  geom_point(pch = 21, position = position_jitterdodge(), alpha = 0.2)+
  geom_boxplot(notch = TRUE, colour = "grey35", alpha = 0.5, linewidth=0.5)+
...
ggsave(filename = here("figures/disaggregation_of_pressures_mass_allocation.jpg"), width = 12, height=8, units = "cm", device = "jpg", dpi = 600)
