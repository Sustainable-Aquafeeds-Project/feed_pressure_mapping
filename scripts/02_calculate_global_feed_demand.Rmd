...
Now we need to work out how the ingredient demand converts to forage fish biomass. This will depend on species which depends on geographic region. So we need different conversions from different species.


```{r}
#bring in total ingredient demand data
...
(fishmeal_conversions_by_species <- read_csv(here("data/raw_data/fishmeal_conversion_by_species.csv")) |> 
  clean_names() |> 
  mutate(fm_cf = landings/fishmeal,
         fo_cf = landings/fish_oil))




(fishmeal_conversion_by_country <- read_csv(here("data/raw_data/fishmeal_conversion_by_country.csv")) |> clean_names()
