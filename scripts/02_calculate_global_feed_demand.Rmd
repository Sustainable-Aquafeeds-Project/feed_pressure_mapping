...
(forage_catch_summary <- forage_catch |> 
  ...
  arrange(-total_catch) |> 
  left_join(embodied_fish_ratios |> filter(Ingredient == "Fishmeal") |> select(-Ingredient) |> rename(fishmeal=`2020`), by = c("TaxonName" = "sci_name")) |> 
    left_join(embodied_fish_ratios |> filter(Ingredient == "Fish oil") |> select(-Ingredient) |> rename(fish_oil=`2020`), by = c("TaxonName" = "sci_name", "common_name", "ecosystem_combined"))
)


#plot forage catch to identify main regions for production. The areas I'm interested in is the Humboldt system, the caribeban coast off latin america, the gulf of mexico, the north atlantic, west africa, and the east asia regions
...
fao_areas <- c(87, 31, 27, 34, 61) #these fao areas broadly correspond with the area of most interest above
...
ggplot()+
  geom_sf(data = forage_regions_shp |> mutate(f_area = factor(F_AREA)), aes(fill = NAME_EN))+
  geom_sf(data = world_shp, size=NA, fill="grey60")+
  labs(fill = "")+
  scale_fill_manual(values = RColorBrewer::brewer.pal(5, name = ))
  new_scale_fill()+
  geom_tile(data = catch_raster, aes(x = x, y = y, fill=log10(z)))+
  theme_bw()
...
forage_catch |> filter(CommonName %in% missing_regions_factors$CommonName) |> select(CommonName, `FAO name`) |> count(CommonName, `FAO name`) |> mutate(region = countrycode(sourcevar = `FAO name`, origin = "country.name", destination = "un.regionsub.name", warn=TRUE))
