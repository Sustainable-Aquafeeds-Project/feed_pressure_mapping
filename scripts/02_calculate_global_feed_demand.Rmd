...
forage_catch <- readRDS(here("data/raw_data/fisheries/spatial_forage_catch_2015.rds"))

(forage_catch_summary <- forage_catch |> 
  group_by(TaxonName, CommonName) |> 
  summarise(total_catch = sum(total_catch, na.rm=TRUE)) |> 
  arrange(-total_catch) |> 
  left_join(embodied_fish_ratios |> filter(Ingredient == "Fishmeal") |> select(-c(Ingredient, ecosystem_combined)) |> rename(fishmeal=`2020`), by = c("TaxonName" = "sci_name")) |> 
    left_join(embodied_fish_ratios |> filter(Ingredient == "Fish oil") |> select(-c(Ingredient, ecosystem_combined)) |> rename(fish_oil=`2020`), by = c("TaxonName" = "sci_name", "common_name", "ecosystem_combined"))
)
...
fao_areas <- c(77, 87, 31, 27, 34, 51, 61) #these fao areas broadly correspond with the area of most interest above

#read in the fao area shapefiles from the FAO json
FAO_regions_shp <- read_sf("https://www.fao.org/fishery/geoserver/fifao/ows?service=WFS&request=GetFeature&version=1.0.0&typeName=fifao:FAO_AREAS_CWP&outputFormat=json") |> filter(F_LEVEL == "MAJOR") |> mutate(F_AREA = as.numeric(F_AREA))

#check FAO shapefile appears correctly
ggplot()+
  geom_sf(data = FAO_regions_shp_1, 
          aes(fill = F_AREA))
...
ggplot()+
  ...
  labs(fill = "")+
  scale_fill_manual(values = RColorBrewer::brewer.pal(7, name = "Blues"))+
   theme_void()+
  theme(
...
FMFO_potential_frm_catch <- forage_catch |> 
  ...
  group_by(fao_area_code, fao_area_name, TaxonName, CommonName) |> 
  summarise(total_catch = sum(total_catch, na.rm=TRUE)) |> 
  arrange(fao_area_name, -total_catch) |> 
  group_by(fao_area_code) |> 
  nest() |> 
  mutate(prop_region_catch = map(data, ~(.$total_catch/sum(.$total_catch)))) |> 
  unnest(cols = c(data, prop_region_catch)) |> 
  ...
  mutate(fishmeal_cf = case_when(is.na(common_name) ~ globalav_embodiedfish_fm,
         ...
         fish_oil_cf = case_when(is.na(common_name) ~ globalav_embodiedfish_fo,
                                 TRUE ~ fish_oil_cf)) |> 
  select(-common_name) |> 
  mutate(fm_potential = total_catch/fishmeal_cf,
         fo_potential = total_catch/fish_oil_cf)
...
supply_vs_demand_byarea <- 
  ...
  summarise(total_fm_supply = sum(fm_potential),
...
FMFO_potential_frm_catch |> 
  ...
  nest() |> 
  mutate(marine_fm_prop_area_supply = map(data, ~(.$marine_diet_fm_demand/sum(.$fm_potential))),
         plant_fm_prop_area_supply = map(data, ~(.$plant_diet_fm_demand/sum(.$fm_potential))),
         marine_fo_prop_area_supply = map(data, ~(.$marine_diet_fo_demand/sum(.$fo_potential))),
         plant_fo_prop_area_supply = map(data, ~(.$plant_diet_fo_demand/sum(.$fo_potential)))) |> 
  unnest(cols = c(data,marine_fm_prop_area_supply, plant_fm_prop_area_supply,marine_fo_prop_area_supply, plant_fo_prop_area_supply))
