...
marine_source_ingredient <- 
  marine_diets |>
  select(1:501) |> 
  rename_with( ~ paste("source", .x)) |>
  mutate(row = row_number()) |> 
  mutate_at(vars(matches("source")), case_when(`source ingredient` == "guar meal"~ as.integer(1))) |>  
  group_split(row) 


sources <- marine_source_ingredient |> select(-c("source ingredient", "row")) |>  names()   
