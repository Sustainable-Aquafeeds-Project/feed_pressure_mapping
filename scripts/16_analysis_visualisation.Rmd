...
marine_source_file_list_ge <- list()


for(source in sources){
    
    marine_diet_files_ge <- 
      map(.x = marine_source_ingredient, .f = \(this_df){
        
        this_allocation_method <- "gross energy allocation"
    ...
    marine_source_file_list_ge[[source]] <- marine_diet_files_ge
...
marine_source_rast_list_ge <- list()
...
for(source in sources){
  ...
  marine_source_rast_list_ge[[source]] <- marine_rast_list_ge
...
marine_diet_raster_ge <- list()

options(warn = 0)
for(source in sources){
  
  #stack rasters for each sourcing combination
  marine_diet_raster_ge[[source]] <- app(rast(marine_source_rast_list_ge[[source]]), sum)
  marine_diet_raster_ge[[source]][marine_diet_raster_ge[[source]]==0] <- NA
  
  #reproject raster 
  marine_diet_raster_ge[[source]] <- project(marine_diet_raster_ge[[source]], moll_crs)
