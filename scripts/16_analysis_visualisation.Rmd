...
marine_pressure_files <- 
  list.files("/mnt/rdsi/github/feed_pressure_mapping/data/spatial/marine_diet/pressures", full.names = TRUE) |> 
  str_extract()
...
#Differences among regions withinmarine-dominant feed
...
marine_source_ingredient <- 
  marine_diets |> 
  select(ingredient) |> 
  mutate(source_1 = c(1, 2, 1, 2, 1, 2),
         source_2 = c(2, 1, 1, 1, 2, 3),
         source_3 = c(3, 3, 1, 3, 3, 1)) |> 
...
plant_source_ingredient <- 
  plant_diets |> 
  select(ingredient) |> 
  mutate(source_1 = c(1, 1, 2, 2, 1, 1, 1, 3, 3, 2, 2, 1),
         source_2 = c(2, 2, 3, 3, 2, 1, 3, 2, 1, 3, 1, 2),
         source_3 = c(3, 3, 1, 1, 3, 1, 2, 1, 2, 1, 3, 3)) |> 
   mutate(row = row_number()) |> 
  group_split(row) 
...
marine_vals[marine_vals>min_plant] |> length()/ length(marine_vals) #86.2% of marine values

#which plant vals are lower than the marine maximum?
plant_vals[plant_vals<max_marine] |> length()/ length(plant_vals) #84.6%

#median comparisons
median(marine_vals)
median(plant_vals)

median(plant_vals)/median(marine_vals) #21% higher median CPI for plants

#maximum comparisons
max(marine_vals)
max(plant_vals)

max(plant_vals)/max(marine_vals) #32% higher maximum CPI for plants


#minimum of one, maximum of anotehr
min(plant_vals)/max(marine_vals) #plant diets can have CPI 63% lower than marine diets
```
...
both_diets_disagg_p <- 
  ...
  labs(y = "Normalised pressure scores")+
