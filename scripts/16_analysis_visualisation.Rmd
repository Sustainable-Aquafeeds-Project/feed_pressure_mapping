...
us_maize_r <- rast(here("data/spatial/marine_diet/production/corn gluten meal_AllTech_CAN.tif"))

us_maize_df <- terra::as.data.frame(us_maize_r, xy =TRUE)

countries_shp <- ne_countries(scale = "medium", returnclass = "sf") |> filter(adm0_a3 ==  "CAN" ) 
...
can_cgm <- rast("/mnt/rdsi/github/feed_pressure_mapping/data/spatial/marine_diet/int/corn gluten meal_ghg_km2_CAN.tif")

cancgm_df <- terra::as.data.frame(can_cgm, xy = TRUE)


ggplot()+
  geom_sf(data = countries_shp, fill="grey95" )+
  geom_tile(data = cancgm_df, aes(x=x, y=y, fill=kgCO2eqkm2))+
  scale_fill_gradientn(colours = rcartocolor::carto_pal(n=7, "SunsetDark"))+
  theme_bw()+
  labs(fill = "GWP\nkg CO2e km-2")
...
fo_ghg <- rast(here("data/spatial/marine_diet/int/fish oil_ghg_raw_marine_diet_NAT.tif"))

fo_ghg_df <-  terra::as.data.frame(fo_disturbance, xy = TRUE)


ggplot()+
  geom_tile(data = fo_ghg_df, aes(x=x, y=y, fill=total_GWP_km2_fo))+
  scale_fill_viridis_c()+
  theme_bw()+
  labs(fill = "GWP\nkg CO2e km-2")
