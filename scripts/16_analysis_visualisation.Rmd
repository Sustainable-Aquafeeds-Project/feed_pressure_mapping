...
both_diets <- bind_rows(marine_diet, plant_diet) |> 
  ...
  mutate(diet = factor(diet, levels = c("Plant-\ndominant", "Marine-\ndominant")))
...
both_diets_p <- ggplot(data = both_diets,
       aes(x = diet , y = prop, fill = ingredients))+
  geom_col()+
  scale_fill_manual(values =  rev(colorRampPalette(colors = rcartocolor::carto_pal(n = 7, name = "Safe"))(16)))+
  theme_pubr()+
  coord_flip()+
  ...
  guides(fill = guide_legend(byrow=TRUE, reverse = TRUE, ncol = 6))


ggsave(filename = here("figures/feed_composition.jpg"), dpi = 600, width = 18, height = 9, units = "cm")
...
ggplot(data = marine_plant_df |> mutate(diet = case_when(diet == "plant diet" ~ "Plant-\ndominant\nfeed",
                                                         diet == "marine diet" ~ "Marine-\ndominant\nfeed"),
                                        diet = factor(diet, levels = c("Plant-\ndominant\nfeed", "Marine-\ndominant\nfeed"))))+
  aes(x = sum, y = diet, colour = diet)+
    geom_jitter(shape =20, size = 5, alpha = 0.1, height = 0.4
  )+
  ...
  theme_pubr()+
  geom_boxplot(notch = TRUE, colour = "grey40", width = 0.3, alpha = 0, size=1.1)+
...
ggsave(filename = here("figures/diet_permutation_comparisons.jpg"), device = "jpg", dpi = 600, width = 8.9, height = 3.8 )
...
marine_vals[marine_vals>min_plant] |> length()/ length(marine_vals) #79% of marine values

#which plant vals are lower than the marine maximum?
plant_vals[plant_vals<max_marine] |> length()/ length(plant_vals) #68.2%
...
disaggregated_pressures_df_list <- list()
...
for(source in sources){
  ...
  disaggregated_pressures_df_list[[source]] <- bind_rows(this_ghg_df, this_water_df, this_N_df, this_P_df, this_disturbance_df)
  
}

marine_diet_pressures_df <- bind_rows(disaggregated_pressures_df_list) |> mutate(diet = "marine diet")
...
plant_diet_pressures_df <- bind_rows(disaggregated_pressures_df_list) |> mutate(diet = "plant diet")
...
summarised_pressure_disagg_df <- 
  ...
  summarise(mean = mean(sum, na.rm=TRUE),
            ...
            lq = quantile(sum, 0.25),
            uq = quantile(sum, 0.75)) |> 
...
ggplot(data= summarised_pressure_disagg_df, aes(x = pressure, y = mean, fill = diet)
  ...
  geom_col( position = "dodge", width = 1, alpha = 0.8)+
...
Disaggreagte by ingredients and pressures -  Marine diets
...
marine_ingredients <- paste0(c(readRDS(here("data/tidy_data/demand/total_ingredient_demand.rds")) |> filter(diet=="marine_diet" & total_ingredient_demand>0 & (!ingredients %in% c("fish oil-wild", "fishmeal-wild"))) |> pull(ingredients), "forage_fish"), "_")
...
plant_ingredients <- paste0(c(readRDS(here("data/tidy_data/demand/total_ingredient_demand.rds"))  |> filter(diet=="plant_diet" & total_ingredient_demand>0 & (!ingredients %in% c("fish oil-wild", "fishmeal-wild", "canola/camelina oil"))) |> pull(ingredients), "forage_fish", "canola oil"), "_")
...
summarised_ingredient_pressure_df <- 
  ...
  summarise(mean = mean(sum, na.rm=TRUE),
            ...
            median = median(sum, na.rm = TRUE),
            lq = quantile(sum, 0.25),
            uq = quantile(sum, 0.75), .groups = "drop") |> 
  mutate(diet = if_else(diet == "marine diet", true = "Marine-dominant feed", false = "Plant-dominant feed")) |> 
...
marine_diet <- 
  ggplot(data = summarised_ingredient_pressure_df |> 
         ...
         aes(x = pressure, y = mean, fill=ingredient))+
  # geom_polygon(aes(group = ingredient, colour = ingredient), alpha=0.3, size=1.1)+
  coord_polar(clip="off")+
  
  geom_col(aes(fill=ingredient), position = "dodge", width = 0.75, alpha = 0.8)+
  geom_point(position = position_dodge(width = 0.7), size=0.1, colour="red")+
  geom_errorbar(aes(x = pressure, ymin = mean-sd, ymax = mean+sd), position = position_dodge(width=0.7), width = 0, size = 0.2)+
  #geom_point(aes(x=pressure, y = mean), colour = "red", shape = 5, position = position_dodge(width = 0.7), size=0.4)+
  #scale_fill_manual(values = rcartocolor::carto_pal(n=6, name = "Safe")[c(5,11,2, 4, 3,7)])+
  scale_fill_manual(values = rcartocolor::carto_pal(n=12, name = "Safe"))+
  ...
  theme(panel.border = element_blank(),
        ...
        text = element_text(size=8))+
  labs(y = "Normalised pressure index", fill = "Marine-dominant feed")+
  guides(fill = guide_legend(override.aes = list(shape = NA), title.hjust = 0.5, title.position = "top"))


ggsave(here("figures/marine_disaggreation_by_ingredient_pressure.jpg"), dpi = 600, height=9.4, width=8.9, units="cm")
...
plant_diet <- 
  ...
  coord_polar(clip="off")+
  geom_col(aes(fill=ingredient), position = "dodge", width = 0.75, alpha = 0.8)+
  geom_point(position = position_dodge(width = 0.75), size=0.1, colour="red")+
  geom_errorbar(aes(x = pressure, ymin = mean-sd, ymax = mean+sd), position = position_dodge(width=0.75), width = 0, size = 0.2)+
  #geom_point(aes(x=pressure, y = mean), colour = "red", shape = 5, position = position_dodge(width = 0.7), size=0.4)+
  scale_fill_manual(values = rcartocolor::carto_pal(n=12, name = "Safe"))+
  #scale_fill_manual(values = rcartocolor::carto_pal(n=12, name = "Safe")[c(10,5, 6, 8, 11, 9, 2, 12, 4,1, 3,7)], labels = str_pad_custom)+
  ...
  theme(panel.border = element_blank(),
        ...
        legend.box.spacing = unit(-0.2, "cm"),
        text = element_text(size=8))+
  labs(y = "Normalised pressure index", fill = "Plant-dominant feed")+
  guides(fill = guide_legend(override.aes = list(shape = NA), title.hjust = 0.5, title.position = "top"))


ggsave(here("figures/plant_disaggreation_by_ingredient_pressure.jpg"), dpi = 600, height=9.4, width=8.9, units="cm")


 
joint_legend <- get_legend(marine_diet)


(marine_diet|plant_diet)

ggsave(here("figures/disaggregation_of_pressure_by_ingredient.jpg"), dpi = 600, width = 18, height =11, units="cm")
