...
#demand exceeds average production in 12% of instances 
...
AllocateSpatialCropDemand <- \(this_ingredient_source){
  ...
  if(file.exists(this_demand_raster_filename)){
    ...
    #using all technologies at present - may want to disaggregate by rainfed or irrigated
    this_global_area_raster <- rast(list.files(here("data/spatial/02-crop-layers-reprojected"), pattern = sprintf("_A_%s_A", toupper(this_mapspam_code)), full.names = TRUE))
    ...
    this_demand_production_ratio <- this_demand_quantity/sum(terra::values(this_country_production_raster), na.rm = TRUE)
...
top_10_producers |> filter(item == "Rapeseed")
