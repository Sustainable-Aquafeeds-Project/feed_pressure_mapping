...
#what proportion of CPI does processing represent on average across irrespective of diet?
...
#what proportion of CPI does processing represent on average for the marine diet?
processing_production_prop |> 
...
#what proportion of CPI does processing represent on average for the plant diet?
processing_production_prop |> 
...
fo_dist_plot <- 
  ...
  annotation_custom2(ff_png, xmin = 2, xmax = 3.5, ymin = 460, ymax = 500, data = fo_disturbance_df |> filter(diet == "Fish-dominant"))
...
fo_ghg_plot <- 
  ...
  geom_text(data = facet_text, aes(x = area_name, y = sum), label = "Fish oil", size=3, fontface="bold")+
  annotation_custom2(ff_png, xmin = 2, xmax = 3.5, ymin = 202000, ymax = 212000, data = fo_disturbance_df |> filter(diet == "Fish-dominant"))
...
marine_faba_water <- 
  ...
  list.files(here("data/spatial/marine_diet/int"), 
                        pattern = "faba beans_production_water_moll|faba beans_processing_water_moll", full.names = TRUE) |>
  map(.f = \(this_file){
...
faba_water_plot <- 
  ...
  geom_text(data = facet_text, aes(x = area_name+0.5, y = sum), label = "Faba beans", size=3, fontface="bold")+
 annotation_custom2(faba_png, xmin = 2.7, xmax = 4.9, ymin = 135, ymax = 150, data = faba_water_df |> filter(diet == "Fish-dominant"))
...
(fo_ghg <- 
  ...
  group_by(diet, area_name) |> summarise(sum = sum(sum)) |> 
  mutate(ingredient_tonnes = case_when(diet == "Fish-dominant" ~ ingredient_demand |> 
                                                     filter(ingredients == "fish oil" & diet == "marine_diet") |> 
                                         pull(ingredient_demand_tonnes),
                                       diet== "Plant-dominant" ~ ingredient_demand |> 
                                                     filter(ingredients == "fish oil" & diet == "plant_diet") |> 
                                         pull(ingredient_demand_tonnes))) |> 
  mutate(ghg_tonne = sum/ingredient_tonnes) |> 
  ungroup() |> 
  group_by(diet) |> 
...
(soy_dist <- 
  ...
  mutate(ingredient_tonnes = case_when(diet == "Fish-dominant" ~ ingredient_demand |> 
                                       ...
                                       diet== "Plant-dominant" ~ ingredient_demand |> 
                                                     filter(ingredients == "soy protein concentrate" & diet == "plant_diet") |> 
                                         pull(ingredient_demand_tonnes))) |> 
  mutate(km_tonne = sum/ingredient_tonnes) |> 
  ungroup() |> 
  group_by(diet) |> 
  nest() |> 
  mutate(mean = map(data, ~(mean(.$km_tonne)))) |> 
  unnest(cols = c(data, mean))
)
...
ggsave(filename = sprintf(here("explore/ghg_marine_vs_plant\_%s.jpg"), this_allocation_method), dp = 600, height = 7, width=18, unit = "cm")
