...
fo_dist_plot <- 
  ggplot(data = fo_disturbance_df, aes(x = diet, y = sum, fill = area_name, pattern = stage))+
  ...
  theme(legend.position = c(0.75,0.82),
        ...
        legend.background = element_rect(fill = "transparent"))+
  labs(fill = "FAO fishing area", y = bquote(Disturbance~(km^2~eq.)), x = "Feed scenario")
...
fo_ghg_plot <- 
  ggplot(data = fo_GHG_df, aes(x = diet, y = sum/1000, fill = area_name, alpha = stage))+
...
WG_nutr_df <- bind_rows(marine_WG_nutr, plant_WG_nutr) |> 
  ...
  #theme(plot.margin = unit(c(1,1,1,0), "lines"))+
    annotate("segment", x = 3, xend = 3, y=-5, yend = -2.2)
