...
marine_fo_dist <- 
  ...
  list.files(here("data/spatial/marine_diet/int"), 
                        pattern = "fish oil_production_disturbance_moll", full.names = TRUE) |>
  map(.f = \(this_file){
...
fo_dist_plot <- 
  ggplot(data = fo_disturbance_df, aes(x = diet, y = sum, fill = area_name))+
  ...
  theme(legend.position = c(0.75,0.82),
        ...
        legend.background = element_rect(fill = "transparent"))+
  labs(fill = "FAO fishing area", y = bquote(Fishing~disturbance~(km^2~eq.)), x = "Feed scenario")+
  guides(fill = "none")
...
marine_fo_GHG <- 
  ...
  list.files(here("data/spatial/marine_diet/int"), 
                        pattern = "fish oil_production_ghg_moll", full.names = TRUE) |>
  map(.f = \(this_file){
...
fo_ghg_plot <- 
  ggplot(data = fo_GHG_df, aes(x = diet, y = sum/1000, fill = area_name))+
  geom_bar(stat = "identity", position = "dodge")+
  scale_fill_manual(values = rcartocolor::carto_pal(n=5, name = "TealGrn"))+
  theme_pubr()+
  ...
  theme(legend.position = c(0.82,0.82),
        ...
        legend.key.size = unit(0.4, "cm"),
        legend.background = element_rect(fill = "transparent"))+
  labs(fill = "FAO fishing area", y = bquote(GHG~emissions~(tonnes~CO[2]~eq.)), x = "Feed scenario")
...
marine_SBM_dist <- 
  ...
  list.files(here("data/spatial/marine_diet/int"), 
                        pattern = "soybean meal_production_disturbance_moll", full.names = TRUE) |>
  map(.f = \(this_file){
...
plant_SPC_dist <- 
  ...
  list.files(here("data/spatial/plant_diet/int"), 
                        pattern = "soy protein concentrate_production_disturbance_moll", full.names = TRUE) |>
  map(.f = \(this_file){
...
soy_dist_plot <- 
  ggplot(data = soy_disturbance_df, aes(x = diet, y = sum, fill = area_name))+
  geom_bar(stat = "identity", position = "dodge")+
  ...
  labs(fill = "Soy producer", y = bquote(Disturbance~(km^2~eq.)), x = "Feed scenario")
...
marine_SBM_GHG <- 
  ...
  list.files(here("data/spatial/marine_diet/int"), 
                        pattern = "soybean meal_production_ghg_moll", full.names = TRUE) |>
  map(.f = \(this_file){
...
plant_SPC_GHG <- 
  ...
  list.files(here("data/spatial/plant_diet/int"), 
                        pattern = "soy protein concentrate_production_ghg_moll", full.names = TRUE) |>
  map(.f = \(this_file){
...
soy_ghg_plot <- 
  ggplot(data = soy_ghg_df, aes(x = diet, y = sum/1000, fill = area_name))+
  geom_bar(stat = "identity", position = "dodge")+
  scale_fill_manual(values = rcartocolor::carto_pal(n=5, name = "BrwnYl"))+
  theme_pubr()+
  ...
  theme(legend.position = c(0.85,0.82),
        ...
        legend.key.size = unit(0.4, "cm"),
        legend.background = element_rect(fill = "transparent"))+
  labs(fill = "Soy producer", y = bquote(GHG~emissions~(tonnes~CO[2]~eq.)), x = "Feed scenario")
...
WG_nutr_df <- bind_rows(marine_WG_nutr, plant_WG_nutr) |> 
  ...
  ggplot(data = WG_nutr_df, aes(x = area_name, fill = area_name, y = sum/1000,  pattern = stage))+
  geom_col_pattern(
    position="stack",
    pattern_type = "stripe",
    pattern_size = 0.08,
  ...
  labs(fill = "Wheat producer", y = bquote(Excess~nutrients~(tonnes~N/P~eq.)), x = "Feed scenario", pattern = "Stage")+
  guides(fill = guide_legend(override.aes = list(pattern = "none"), title.position = "top"),
         pattern = guide_legend(override.aes = list(fill = "grey95"), title.position = "top"))+
...
ggsave(filename = sprintf(here("explore/wheat_gluten_stage_comparisons_nurients_%s.jpg"), this_allocation_method), dpi = 600, width = 8.9, height=7, unit="cm")
