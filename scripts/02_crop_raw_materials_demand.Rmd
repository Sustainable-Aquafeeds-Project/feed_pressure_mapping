...
#get gross energy data and join with conversion factors to get embodied allocation factors
...
#join product and coproduct conversion data
all_cf_data <- 
  cf_data |> 
  select(-X7) |> 
...
dat <- ge_list$data[1] |> as.data.frame()

unique(dat$gross_energy_product)/sum(unique(dat$energy_cf_product), sum(dat$energy_cf_coproduct))
