...
#fishmeal label
facet_text <- data.frame(diet= "Fish-dominant", sum = Inf, area_name = 0.5)
...
fm_dist_plot <- 
  ...
  guides(fill = guide_legend(override.aes = list(pattern = "none"), title.position = "top",order = 2),
   ...
   theme(axis.text.x = element_blank(),
        ...
        legend.background = element_rect(fill = "transparent"),
        legend.margin = margin(-0,0,-0,-0.2, unit="cm"))+
  ...
  geom_text(data = facet_text, aes(x = area_name, y = sum), label = "Fishmeal (32/15%)", size=2.5, fontface="bold", hjust = 0)+
...
soy_dist_plot <- 
  ...
  labs(fill = "Country", y = bquote(Disturbance~(km^2~eq.)), x = "", pattern = "Stage")+
   annotate("segment", x = 3, xend = 3, y=-30, yend = -43)+
  geom_text(data = facet_text, aes(x = area_name, y = sum), label = "SBM/SPC (17/20%)", size=2.5, fontface="bold", hjust =0)+
...
wg_nutr_plot <- 
  ...
  geom_text(data = facet_text, aes(x = area_name, y = sum), label = "Wheat gluten (13/10%)", size=2.5, fontface="bold", hjust =0)+
  annotation_custom2(wheat_png, xmin = -0.2, xmax = 2, ymin = 63, ymax = 75, data = soy_disturbance_df |> filter(diet == "Fish-dominant"))
...
faba_water_plot <- 
  ...
  geom_text(data = facet_text, aes(x = area_name, y = sum), label = "Faba beans (2/3%)", size=2.5, fontface="bold", hjust =0)+
 annotation_custom2(faba_png, xmin = -0.3, xmax = 1.9, ymin = 105, ymax = 120, data = faba_water_df |> filter(diet == "Fish-dominant"))
