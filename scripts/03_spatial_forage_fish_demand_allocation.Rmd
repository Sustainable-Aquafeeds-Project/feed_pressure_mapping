...
Create an FMFO embodied fish demand by source and ingredient for supplementary information
...
embodied_fish_by_dietsource |> 
  mutate(diet = if_else(condition = diet == "marine_diet", true = "Marine diet", false = "Plant diet")) |> 
  ggplot()+
...
#THIS IS NO LONGER NEEDED AFTER TAKING AN DIFFERENT APPROACH WHICH DOES NOT REQUIRE REDISTRIBUTION
...
embodied_fish_demand_by_diet_source <- 
  ...
  map_df(\(this_df){
    ...
    return(tibble(diet = unique(this_df$diet), source_code = unique(this_df$source_code), 
                  embodied_fish_demand_mass = sum(this_df$adj_embodied_fish_mass), 
                  embodied_fish_demand_ge = sum(this_df$adj_embodied_fish_ge)))
