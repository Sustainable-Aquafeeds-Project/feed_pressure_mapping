...
(plausible_crop_sourcing_list <- crop_demand_production_compare |> select(ingredients, diet, FAOSTAT_name, source_iso3c, total_crop_demand, national_production, demand_production_ratio, check) |>  filter(check == FALSE & total_crop_demand>0) |> group_split(ingredients))
...
AllocateSpatialCropDemand <- \(this_ingredient_source){
  ...
  this_demand_raster_filename <- here(sprintf("data/spatial/%s/production/%s_AllTech_%s.tif", this_diet, this_ingredient, this_country))
  
  message("processing ", this_diet, ", ", this_ingredient, ", ", this_country)
