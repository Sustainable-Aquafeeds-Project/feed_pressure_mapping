...
embodied_fish_new_gear <- 
  ...
  as_tibble() |> 
  mutate(fm_adj_embodied_fish_kg = embodied_fish_adj_fm *1000,
         fo_adj_embodied_fish_kg = embodied_fish_adj_fo *1000) |> 
  mutate(ei_kg_CO2_eq_fm = fm_adj_embodied_fish_kg*ei_kgco2_kgcatch,
         ei_kg_CO2_eq_fo = fo_adj_embodied_fish_kg*ei_kgco2_kgcatch)
...
embodied_fish_new_gear |> 
  select(diet, source_code, embodied_fish_adj_fo, embodied_fish_adj_fm) |> 
  group_by(diet, source_code) |> 
  summarise(fo_embedded = sum(embodied_fish_adj_fo, na.rm=TRUE),
            fm_embedded = sum(embodied_fish_adj_fm, na.rm = TRUE))
...
ghg_list_fm <- embodied_fish_new_gear |> 
  select(diet, source_code, LonCentre, LatCentre, ei_kg_CO2_eq_fm) |> 
...
ghg_raw_r_list_fm <- 
  ...
  saveName <- sprintf(here("data/spatial/%s/int/fishmeal_ghg_raw_%s_%s.tif"), this_diet, this_diet, this_source)
...
ghg_list_fo <- embodied_fish_new_gear |> 
  select(diet, source_code, LonCentre, LatCentre, ei_kg_CO2_eq_fo) |> 
  group_by(diet, source_code, LonCentre, LatCentre) |> 
...
ghg_raw_r_list_fo <- 
  ...
  saveName <- sprintf(here("data/spatial/%s/int/fish oil_ghg_raw_%s_%s.tif"), this_diet, this_diet, this_source)
