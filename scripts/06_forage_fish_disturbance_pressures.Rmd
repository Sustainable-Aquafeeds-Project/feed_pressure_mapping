...
map(trawl_forage_catch_list, \(this_df){
  ...
  this_summary_df_mass <- 
    this_df |> 
    group_by(LonCentre, LatCentre) |> 
    summarise(forage_catch_mass = sum(embodied_fish_adj_fm_mass, na.rm=TRUE))
  ...
  #Gross energy allocation
   this_summary_df_ge <- 
    this_df |> 
    group_by(LonCentre, LatCentre) |> 
    summarise(forage_catch_ge = sum(embodied_fish_adj_fm_ge, na.rm=TRUE))
...
fm_effort_files_pd <- fm_effort_files_int_pd[!grepl("0.5D_km2", fm_effort_files_int_pd) & grepl("_allocation", fm_effort_files_int_md)]
...
fm_catch_list <- fread(here("data/tidy_data/demand/embodied_fish_per_cell.csv")) |> 
  ...
  summarise(mass_allocation = sum(embodied_fish_adj_fm_mass, na.rm=TRUE),
            ge_allocation = sum(embodied_fish_adj_fm_ge, na.rm=TRUE)) |> 
  as_tibble() |> 
...
combined_raster_list <- 
  ...
  writeRaster(x = this_km2_combined_r, filename = sprintf(here("data/spatial/marine_diet/int/fishmeal_disturbance_km2_%s_%s.tif"), this_source, this_allocation_method), overwrite=TRUE)
...
combined_raster_list2 <- 
  map2(.x = fm_benthic_destruction_pd, .y = fm_biomass_removal_pd, .f = \(this_benthic_file, this_biomass_file){
    ...
    writeRaster(x = this_km2_combined_r, filename = sprintf(here("data/spatial/plant_diet/int/fishmeal_disturbance_km2_%s_%s.tif"), this_source, this_allocation_method), overwrite=TRUE)
...
map(trawl_forage_catch_list, \(this_df){
  ...
  this_summary_df_mass <- 
    this_df |> 
    group_by(LonCentre, LatCentre) |> 
    summarise(forage_catch_mass = sum(embodied_fish_adj_fo_mass, na.rm=TRUE))
  ...
  #Gross energy allocation
   this_summary_df_ge <- 
    this_df |> 
    group_by(LonCentre, LatCentre) |> 
    summarise(forage_catch_ge = sum(embodied_fish_adj_fo_ge, na.rm=TRUE))
