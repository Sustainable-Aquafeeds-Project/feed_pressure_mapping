...
#findings problems with the plant lits - seems to be all nutrients

test <- plant_source_file_list[[1]][4] |> rast() |> ext()

for(i in 1:length(plant_source_file_list)){
  for(j in 1:length(plant_source_file_list[[i]])){
    
  this_file <- plant_source_file_list[[i]][j]
  
  this_rast <- rast(this_file)
  
  if(ext(this_rast)!= test){
    
     message(this_file, "  is a problem")
