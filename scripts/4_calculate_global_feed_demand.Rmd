...
crop_production_raw <- readRDS(file = here("data/tidy_data/crops_production_tidy.rds"))

(crop_production <- 
  crop_production_raw |>
  filter(element == "Production"  & area!= "China" & item %in% ingredient_by_raw_material$FAOSTAT_name & year %in% seq(from = 2015, to = 2020)) |> 
  group_by(area, item) |> 
...
Now we have the top 10 producers for each crop that is used as a raw material in feed, we need to get conversion factors from ingredients to  each of these raw materials in different places. 

```{r}

top_10_crop_producers <- readRDS(here("data/tidy_data/production-data/top_crop_producers.rds"))


arrange(ingredient_by_raw_material, FAOSTAT_name)
```
