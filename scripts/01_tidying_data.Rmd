...
catch_2015 <- fread(file.path(watson_dir, "CatchInd2015_2015.csv")) |> lazy_dt()

codes_cell <- fread(file.path(watson_dir, "codes_cell.csv")) |> lazy_dt()

codes_country <- fread(file.path(watson_dir, "codes_country.csv"))|> lazy_dt()

codes_gear <- fread(file.path(watson_dir, "codes_gear.csv")) |> lazy_dt()

codes_taxa <- fread(file.path(watson_dir, "codes_taxa.csv")) |> lazy_dt()

IndexInd <- fread(file.path(watson_dir, "IndexInd.csv")) |> lazy_dt()


catch_coords_2015 <- catch_2015 |> 
...
Tidy data for forage fish species from Kok et al
```{r}
...
#forage_spp_fao_common <- read_csv(here("data/raw_data/fisheries/fao_forage_fish_spp_list.csv")) |> pull(Name_en) |> unique() 
...
#combine data from different sources and store for future reference
...
#filter the fisheries catch by the forage spp listed by FAO and Kok et al

fisheries_catch <- fread(file.path(watson_dir, "watson_2015_fisheries_catch.csv"))

sort(unique(fisheries_catch$TaxonName))

(forage_catch <- fisheries_catch |> lazy_dt() |> filter(TaxonName %in% forage_spp_list$sci_name | CommonName %in% forage_spp_list$common_name ) |> as_tibble())
...
read_csv(here("data/raw_data/fisheries/forage_fish_list_final.csv"))
