...
The influence of unknown fishing gears ('fishing' above) was addressed in Halpern et al as a source of small source uncertainty. See issue here https://github.com/OHI-Science/global_food_issues/issues/303
...
ext(trawl_effort_r) <- ext(rast())
plot(log10(trawl_effort_r+1))

writeRaster(x = trawl_effort_r, filename =here("data/spatial/03-fisheries-effort/gfw_annual_effort_2017_trawlers.tif"))
...
ext(dredge_effort_r) <- ext(rast())
plot(log10(dredge_effort_r+1))

writeRaster(x = dredge_effort_r, filename = here("data/spatial/03-fisheries-effort/gfw_annual_effort_2017_dredge.tif"))



```

Assign gear classifications to forage catch data i.e. demersal or pelagic, destructive or nondestructive, or high or low bycatch.

In line with the approach from Halpern et al 

Overview:

- This takes a *gear to category* dataset that was created (`gear_to_cat.csv`) manually using information from Table S4 in Halpern et al. (2008), and joined to the raw data. This dataset added three new columns to the catch data; **bycatch** (high or low), **type** (pelagic or demersal), and **destruction** (destructive or nondestructive). 
...
- No catch records with the forage fish catch data had missing or mismatched information.
- Final output is the catch dataset, names `spatial_forage_catch_2017_w_cats.csv` that assigns each taxa+gear type to the categories used to create the fishing stressors.
...
forage_catch_w_cats |> filter(is.na(type))
forage_catch_w_cats |> filter(is.na(bycatch))
```


Also assign gear and destruction types to all catch in the watson dataset
...
plot(npp_r)
writeRaster(x = npp_r, filename = here("data/spatial/annal_mean_npp_2015_gf_wgs.tif"))
