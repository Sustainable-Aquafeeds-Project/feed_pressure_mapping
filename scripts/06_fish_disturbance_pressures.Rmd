...
trim_npp |> filter(fao_area_code == "27" & diet == "marine_diet") |> 
  pull(area_fm_mass)  |> sum()/ (38400*0.3)

#fo trim 
trim_npp |> filter(fao_area_code == "27" & diet == "marine_diet") |> 
  pull(area_fo_mass)  |> sum() /(27600*0.4)


#fishmeal
forage_npp |> filter(fao_area_code == "87" & diet == "marine_diet") |> 
  pull(area_fm_mass)  |> sum() / (38400*0.7)

#fo trim 
forage_npp |> filter(fao_area_code == "87" & diet == "marine_diet") |> 
  pull(area_fo_mass)  |> sum() / (27600*0.6)

total_ingredient_demand_by_diet
...
#fishmeal trim
trim_npp |> filter(fao_area_code == "27" & diet == "marine_diet") |> 
  pull(area_fm_ge)  |> sum()/ (38400*0.3)

#fo trim 
trim_npp |> filter(fao_area_code == "67" & diet == "marine_diet") |> 
  pull(area_fo_ge)  |> sum() /(27600*0.4)


#fishmeal forage
forage_npp |> filter(fao_area_code == "87" & diet == "marine_diet") |> 
  pull(area_fm_ge)  |> sum() / (38400*0.7)

#fo forage 
forage_npp |> filter(fao_area_code == "87" & diet == "marine_diet") |> 
  pull(area_fo_ge)  |> sum() / (27600*0.6)

trim_npp |> pull(fao_area_code) |> unique()




trim_npp|> filter(fao_area_code == "27" & diet == "marine_diet") |> group_by(CommonName) |> summarise(catch = sum(ReportedIND, na.rm = TRUE)) |> arrange(-catch) |> as_tibble()
trim_npp|> filter(fao_area_code == "31" & diet == "marine_diet") |> group_by(CommonName) |> summarise(catch = sum(ReportedIND, na.rm = TRUE)) |> arrange(-catch) |> as_tibble()
trim_npp|> filter(fao_area_code == "67" & diet == "marine_diet") |> group_by(CommonName) |> summarise(catch = sum(ReportedIND, na.rm = TRUE)) |> arrange(-catch) |> as_tibble()
trim_npp|> filter(fao_area_code == "61" & diet == "marine_diet") |> group_by(CommonName) |> summarise(catch = sum(ReportedIND, na.rm = TRUE)) |> arrange(-catch)|> as_tibble()
trim_npp|> filter(fao_area_code == "87" & diet == "marine_diet") |> group_by(CommonName) |> summarise(catch = sum(ReportedIND, na.rm = TRUE)) |> arrange(-catch)|> as_tibble()
```
...
fm_mass_stack <- list.files(here("data/spatial/marine_diet/int"), pattern = "fishmeal_disturbance_km2_mass", full=TRUE) |> rast()

fm_mass_all <- app(fm_mass_stack, sum, na.rm=TRUE)



ggplot()+
  geom_tile(data = fm_mass_all |> 
