...
(forage_npp <- 
    ...
    ungroup() |> 
    mutate(ppr_prop_fm = case_when(ppr_fm/max_npp_t_C_yr<1 ~ ppr_fm/max_npp_t_C_yr,
                                        TRUE ~ ppr_fm/max_npp_t_C_yr),
           
           ppr_prop_fo = case_when(ppr_fo/max_npp_t_C_yr<1 ~ ppr_fo/max_npp_t_C_yr,
...
trim_npp |> filter(fao_area_code == "27" & diet == "marine_diet") |> 
  pull(area_fm)  |> sum()/ (38400*0.3)

#fo trim 
trim_npp |> filter(fao_area_code == "27" & diet == "marine_diet") |> 
  pull(area_fo)  |> sum() /(27600*0.4)


#fishmeal forage
forage_npp |> filter(fao_area_code == "87" & diet == "marine_diet") |> 
  pull(area_fm)  |> sum() / (38400*0.7)

#fo forage
forage_npp |> filter(fao_area_code == "87" & diet == "marine_diet") |> 
  pull(area_fo)  |> sum() / (27600*0.6)
