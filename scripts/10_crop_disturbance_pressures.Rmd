...
demand_production_compare_w_sources <- 
  ...
  this_crop_adj <- this_crop |> filter(source_iso3c %in% sources)
}) |> 
  #filter(!(FAOSTAT_name == "Linseed" & source_iso3c %in% c("CHN", "GBR"))) |> 
  filter(demand_production_ratio_ge>0 | demand_production_ratio_ms>0)
...
scalar_source_list <- 
  ...
  select(-c(ge_allocation_factor, mass_allocation_factor, total_crop_demand_ge, total_crop_demand_ms, check_ge, check_ms)) |> 
...
map(.x = scalar_source_list, \(this_file){
  ...
  saveName <- sprintf(here("data/spatial/%s/int/%s_disturbance_km2_%s_%s.tif"), this_diet, this_ingredient, this_country, this_allocation_method)
  
  if(!file.exists(saveName)){
    
    message("Processing ",  basename(tools::file_path_sans_ext(saveName)))
    ...
    corrected_area_raster <- lapp(c(this_country_crop_demand_rast,cellSize(this_country_crop_demand_rast)), \(x,y){
