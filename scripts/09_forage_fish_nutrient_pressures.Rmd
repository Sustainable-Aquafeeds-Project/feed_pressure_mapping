...
For each diet and source import the rasters from ghg and make all values zero - then rexport into "int" folder with nutrient file names
...
map(.x = template_files, .f = \(this_file){
  ...
  if(!file.exists(saveName_N_md) | !file.exists(saveName_N_pd) | !file.exists(saveName_P_md) | !file.exists(saveName_P_pd)){
