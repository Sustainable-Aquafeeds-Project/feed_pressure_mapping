...
port_number <- map_df(fao_areas_shp, \(this_area){
  ...
  these_ports <- st_intersection(wpi, this_area) |> mutate(fao_area = this_fao_area)
...
map(sourcing_countries, \(this_diet_ingredient_demand){
  ...
  writeRaster(this_centroid_raster, filename = sprintf(here("data/spatial/%s/processing/%s_%s.tif"), this_diet, this_ingredient, this_country), overwrite=TRUE)
  
})


plot(this_centroid_raster)
```
