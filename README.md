...
| large_data | This folder is designed to hold data that is not able to be held in this github repository. Prior to publication, please contact author (richardstuart.cottrell@utas.edu.au) for a private Dropbox link to this folder. Following publication, this folder will be stored in the Susatinable Aquafeed Project's Zenodo community (https://zenodo.org/communities/sustainableaquafeedsproject). All data in the large_data folder should be placed unmodified in the following file path convention within the cloned or forked project: "your_file_path_to_the_project_folder/feed_pressure_mapping/data/large_data", or, using the here library, here("data/large_data").
